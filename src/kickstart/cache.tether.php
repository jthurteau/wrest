...
return function (
    ...
    $fileSafe = #TODO #2.0.0 this can be folded into the Phagent
    ...
    : [
        '?composer',
        //'?host', #NOTE some of these wouldn't make sense in the cache
        "?{$applicationHandle}",
        '?local-instance',
    ]; 
    ...
    if(file_exists($cachePath) && is_readable($cachePath)) {
        ...
        if (
            ...
            #TODO #2.0.0 handle cacheSnapshots missing from canister
            return;
        }
    ...
    if (!is_array($cache) && is_writable(dirname($cachePath))) {
        ...
        try{
            $canisterSnapshot = (function(array $array) {
                $snapshot = [
                    'cacheSnapshots' => [],
                ];
                foreach($array as $key => $value) {
                    if (!is_callable($value)) {
                        ...
                        $snapshot['cacheSnapshots'] = $key;
                    }
                }
                return $snapshot;
            })($canister);
            $cacheString = json_encode($canisterSnapshot, $cacheOptions, $cacheDepth);
